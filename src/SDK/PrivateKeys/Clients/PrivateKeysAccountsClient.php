...
use Virgil\SDK\PrivateKeys\Models\VirgilPrivateKeyAccount;
use Virgil\SDK\PrivateKeys\Models\VirgilPrivateKeysAccountType;
...
class PrivateKeysAccountsClient extends ApiClient implements PrivateKeysAccountsClientInterface {
    ...
    public function getAccount($accountId) {
        ...
        return new VirgilPrivateKeyAccount($response->getBody());
    }
    ...
    public function verifyResetPassword($token) {
        $response = $this->put('account/verify', array(
            'token' => $token
        ));
        ...
        if(!empty($data['data'])) {
            foreach($data['data'] as $item) {
                ...
                $privateKeysCollection->add(new VirgilPrivateKey($item));
            }
        }
